...
public class ArbolB {
    ...
    public void localidades(int num, double latitud, double longitud){
        this.latitud = latitud;
        this.longitud = longitud;
        
        int valor = searchLocalidades(root, num);
    ...
    //Buscar
    //private nodoB search(nodoB actual, int key){
    private int searchLocalidades(nodoArbolB actual, int key){
        int i = 0;//se empieza a buscar siempre en la primera posicion
        ...
        if(i < actual.n && key == actual.key[i].id){
            ...
            if(actual.leaf){
                ...
                //si tiene hijos, hace una llamada recursiva
                return searchLocalidades(actual.child[i], key);
            }
        }
    }
